...
//    fin.close();
    ...
    // 8. generate .bin
    cmdss.clear();
    cmdss << ".\\elf2bin.exe " << "convert amebapro2_firmware.json FIRMWARE firmware_ntz.bin";
    getline(cmdss, cmd);
    cout << cmd << endl;
    system(cmd.c_str());
    ...
    cmdss.clear();
    cmdss << ".\\elf2bin.exe " << "combine amebapro2_partitiontable.json flash_ntz.bin PT_PT=partition.bin,CER_TBL=certable.bin,KEY_CER1=certificate.bin,PT_BL_PRI=boot.bin,PT_FW1=firmware.bin,PT_ISP_IQ=firmware_isp_iq.bin,PT_FCSDATA=boot_fcs.bin";
    getline(cmdss, cmd);
    cout << cmd << endl;
    system(cmd.c_str());
