This Helix MP3 example is used to play MP3 files from SD card. In order to run the example the following steps must be followed.
Helix MP3 example

Description:
Helix MP3 decoder is an MP3 decoder which support MP3 file without ID3 tag.
It has low CPU usage with low memory. This example adds an ID3 tag detector to deal with MP3 file with ID3 tag.
...
[Supported List]
	Supported :
	    Ameba-D(when #define AUDIO_SOURCE_SD_CARD (1))
