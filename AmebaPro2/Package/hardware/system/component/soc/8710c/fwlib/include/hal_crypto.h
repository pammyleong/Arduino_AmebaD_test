...
#define _ERRNO_CRYPTO_PROCESS_HDN_DATA_WRITE_KEY_READFAIL       -40
#define _ERRNO_CRYPTO_PROCESS_HDN_DATA_WRITE_KEY_FAIL           -41
#define _ERRNO_CRYPTO_PROCESS_HDN_DATA_WRITE_KEY_OVER_ONCE      -42
#define _ERRNO_CRYPTO_PROCESS_HDN_DATA_NULL_POINTER             -43
#define _ERRNO_CRYPTO_PROCESS_HDN_DATA_NON_SUPPORT_TYPE         -44

typedef enum _PROCESS_SECDATA_HDN_SEC_SEL_TYPE_ {
    WRITE_HDN_KEY1_16BYTES 	= 0,
    WRITE_HDN_KEY2_16BYTES 	= 1,
    WRITE_HDN_KEY1_32BYTES 	= 2
} PROCESS_SECDATA_HDN_SEC_SEL;

typedef enum _PROCESS_SECDATA_OPT_TYPE_ {
    P_SEC_AES_ECB 	= 0,
	P_SEC_AES_CBC 	= 1,
	P_SEC_AES_CFB 	= 2,
	P_SEC_AES_OFB 	= 3,
...
typedef enum _PROCESS_SECDATA_KEY_SEL_TYPE_ {
    KEY_0_SEC_AES_128 	= 0,
    KEY_0_SEC_AES_192 	= 1,
    KEY_0_SEC_AES_256 	= 2,
	KEY_1_SEC_AES_128 	= 3
} PROCESS_SECDATA_KEY_SEL;

typedef enum _PROCESS_SECDATA_CIPHER_ENC_TYPE_ {
    PROCESS_SECDATA_CIPHER_DEC 	= 0,
	PROCESS_SECDATA_CIPHER_ENC 	= 1
} PROCESS_SECDATA_CIPHER_ENC_CTRL;

typedef struct _SECDATA_OPERATE_TYPE_ {
    PROCESS_SECDATA_OPT              cipher_mode;
    PROCESS_SECDATA_KEY_SEL          key_sel;
    PROCESS_SECDATA_CIPHER_ENC_CTRL  enc;
} SECDATA_OPERATE;

typedef struct _PROCESS_SECDATA_CIPHER_FUNC_{
    int (*cipher_init)    (const u8 *key, const u32 keylen);
    int (*cipher_encrypt) (const u8 *message, const u32 msglen,
                           const u8 *iv, const u32 ivlen,
                           u8 *pResult);
    int (*cipher_decrypt) (const u8 *message, const u32 msglen,
                           const u8 *iv, const u32 ivlen,
                           u8 *pResult);
    int (*cipher_ext1_encrypt) (const u8 *message, const u32 msglen,
                                const u8 *iv, u8 *pResult);
    int (*cipher_ext1_decrypt) (const u8 *message, const u32 msglen,
                                const u8 *iv, u8 *pResult);
    int (*cipher_ext2_encrypt) (const u8 *message, const u32 msglen,
                                const u8 *iv,
                                const u8 *aad, const u32 aadlen,
                                u8 *pResult, u8 *pTag);
    int (*cipher_ext2_decrypt) (const u8 *message, const u32 msglen,
                                const u8 *iv,
                                const u8 *aad, const u32 aadlen,
                                u8 *pResult, u8 *pTag);
        
} PROCESS_SECDATA_CIPHER_FUNC;

typedef struct _PROCESS_SECDATA_CIPHER_INPUT_TYPE_ {
    u8 *iv;
    u8 *msg;
    u32 msglen;
    u8 *aad;
    u32 aadlen;
} PROCESS_SECDATA_CIPHER_INPUT_TYPE;

typedef struct _PROCESS_SECDATA_CIPHER_OUTPUT_TYPE_ {
    u8 *pResult;
    u8 *pTag;
} PROCESS_SECDATA_CIPHER_OUTPUT_TYPE;

typedef struct _PROCESS_SECDATA_TYPE_ {
    PROCESS_SECDATA_OPT                opt;
    PROCESS_SECDATA_CIPHER_FUNC        pro_func;
    PROCESS_SECDATA_CIPHER_INPUT_TYPE  pro_input;    
    PROCESS_SECDATA_CIPHER_OUTPUT_TYPE pro_output;
} PROCESS_SECDATA_TYPE;
...
int NS_ENTRY hal_crypto_write_hdn_key_nsc (PROCESS_SECDATA_HDN_SEC_SEL hdn_sec_sel, u8 *pContent);
int NS_ENTRY hal_crypto_process_secdata_nsc(SECDATA_OPERATE *psecdata_ope,
                                            PROCESS_SECDATA_CIPHER_INPUT_TYPE *psecdata_in,
                                            PROCESS_SECDATA_CIPHER_OUTPUT_TYPE *psecdata_out);

#elif defined(CONFIG_BUILD_NONSECURE)
void hal_crypto_engine_init_platform_nsc(const int en);

int hal_crypto_write_hdn_key_nsc (PROCESS_SECDATA_HDN_SEC_SEL hdn_sec_sel, u8 *pContent);

int hal_crypto_process_secdata_nsc(SECDATA_OPERATE *psecdata_ope,
                                   PROCESS_SECDATA_CIPHER_INPUT_TYPE *psecdata_in,
                                   PROCESS_SECDATA_CIPHER_OUTPUT_TYPE *psecdata_out);
...
int hal_crypto_write_hdn_key1 (u8 *pContent);

int hal_crypto_write_hdn_key2 (u8 *pContent);

int hal_crypto_write_hdn_keyall (u8 *pContent);

int hal_crypto_process_secdata_aes_ecb(PROCESS_SECDATA_KEY_SEL key_sel, PROCESS_SECDATA_CIPHER_ENC_CTRL enc,
                                       u8 *iv, u8 *msg, u32 msglen, u8 *pResult);

int hal_crypto_process_secdata_aes_cbc(PROCESS_SECDATA_KEY_SEL key_sel, PROCESS_SECDATA_CIPHER_ENC_CTRL enc,
                                       u8 *iv, u8 *msg, u32 msglen, u8 *pResult);

int hal_crypto_process_secdata_aes_cfb(PROCESS_SECDATA_KEY_SEL key_sel, PROCESS_SECDATA_CIPHER_ENC_CTRL enc,
                                       u8 *iv, u8 *msg, u32 msglen, u8 *pResult);

int hal_crypto_process_secdata_aes_ofb(PROCESS_SECDATA_KEY_SEL key_sel, PROCESS_SECDATA_CIPHER_ENC_CTRL enc,
                                       u8 *iv, u8 *msg, u32 msglen, u8 *pResult);

int hal_crypto_process_secdata_aes_ctr(PROCESS_SECDATA_KEY_SEL key_sel, PROCESS_SECDATA_CIPHER_ENC_CTRL enc,
                                       u8 *iv, u8 *msg, u32 msglen, u8 *pResult);

int hal_crypto_process_secdata_aes_gctr(PROCESS_SECDATA_KEY_SEL key_sel, PROCESS_SECDATA_CIPHER_ENC_CTRL enc,
                                        u8 *iv, u8 *msg, u32 msglen, u8 *pResult);

int hal_crypto_process_secdata_aes_gcm(PROCESS_SECDATA_KEY_SEL key_sel, PROCESS_SECDATA_CIPHER_ENC_CTRL enc,
                                       u8 *iv, u8 *msg, u8 *aad, u32 aadlen, u32 msglen, u8 *pResult, u8 *pTag);
#endif
