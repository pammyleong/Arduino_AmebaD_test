 ...
 * @{
 **/
#define IPSEC_BIT_DES_ERR5                    ((u32)0x00000001 << 15)          /*!<R/W 0  Destination Descriptor Error 5 Interrupt. The error condition is authentication digest length error in destination descriptor. When set sequential hash: MD5: adl != 16 or SHA1: adl != 20 or SHA2-224: adl != 28 or SHA2-256: adl != 32 or SHA2-384: adl != 48 or SHA2-512: adl != 64 Write "1" to clear this error signal. */
#define IPSEC_BIT_DES_ERR4                    ((u32)0x00000001 << 14)          /*!<R/W 0  Destination Descriptor Error 4 Interrupt. The error condition is authentication digest length error in destination descriptor. MD5: adl != 16 or SHA1: adl != 20 or SHA2-224: adl != 28 or SHA2-256: adl != 32 or SHA2-384: adl != 48 or SHA2-512: adl != 64 Write "1" to clear this error signal. */
#define IPSEC_BIT_DES_ERR3                    ((u32)0x00000001 << 13)          /*!<R/W 0  Destination Descriptor Error 3 Interrupt. One of the error condition is the sum of enl is greater than Encryption Total Length. The other error condition is about destination descriptor length error ((enl(Encryption length)==0) & (adl(authentication data length)==0)). Write "1" to clear this error signal. Sum of enl reference the register REG_IPSCSR_SENLR (offset 0x24). */
#define IPSEC_BIT_DES_ERR2                    ((u32)0x00000001 << 12)          /*!<R/W 0  Destination Descriptor Error 2 Interrupt. The error condition is the bit field ENC=1'b0( active Hash engine to process hash data), but only active Cipher engine. Write "1" to clear this error signal. */
#define IPSEC_BIT_DES_ERR1                    ((u32)0x00000001 << 11)          /*!<R/W 0  Destination Descriptor Error 1 Interrupt. The error condition is the bit field ENC=1'b1( active cipher engine to process cipher data), but only active Hash engine. Write "1" to clear this error signal. */
#define IPSEC_BIT_DES_ERR0                    ((u32)0x00000001 << 10)          /*!<R/W 0  Destination Descriptor Error 0 Interrupt. One of the error condition is destination descriptor starting address error (DDSA[1:0]!=2'd0 [At least 4 bytes-aligned]). The other error condition is memory protection event. Write "1" to clear this error signal. */
#define IPSEC_BIT_SRC_ERR9                    ((u32)0x00000001 << 9)          /*!<R/W 0  Source Descriptor Error 9 Interrupt. The error condition is sum of epl is not equal to EPTL(Encryption Padding Total Length). Write "1" to clear this error signal. Sum of epl reference the register REG_IPSCSR_SEPLR (offset 0x2C). */
#define IPSEC_BIT_SRC_ERR8                    ((u32)0x00000001 << 8)          /*!<R/W 0  Source Descriptor Error 8 Interrupt. The error condition is sum of apl is not equal to HPTL(Hash Padding Total Length). Write "1" to clear this error signal. Sum of apl reference the register REG_IPSCSR_SAPLR (offset 0x28). */
#define IPSEC_BIT_SRC_ERR7                    ((u32)0x00000001 << 7)          /*!<R/W 0  Source Descriptor Error 7 Interrupt. The error condition is sum of enl is not equal to ETL(Encryption Total Length). Write "1" to clear this error signal. Sum of enl reference the register REG_IPSCSR_SENLR (offset 0x24). */
#define IPSEC_BIT_SRC_ERR6                    ((u32)0x00000001 << 6)          /*!<R/W 0  Source Descriptor Error 6 Interrupt. The error condition is sum of a2eo is not equal to HTL(Header Total Length). Write "1" to clear this error signal. Sum of a2eo reference the register REG_IPSCSR_SAADLR (offset 0x20). */
#define IPSEC_BIT_SRC_ERR5                    ((u32)0x00000001 << 5)          /*!<R/W 0  Source Descriptor Error 5 Interrupt. The error condition is there is a source descriptor, but the data length( (a2eo/epl) +(enl) + (apl)) is 0. Write "1" to clear this error signal. */
#define IPSEC_BIT_SRC_ERR4                    ((u32)0x00000001 << 4)          /*!<R/W 0  Source Descriptor Error 4 Interrupt. The error condition is source data buffer pointer error(SDBP[1:0]!=2'd0[At least 4 byte aligned]). Write "1" to clear this error signal. */
#define IPSEC_BIT_SRC_ERR3                    ((u32)0x00000001 << 3)          /*!<R/W 0  Source Descriptor Error 3 Interrupt. About source descriptor length error. One of the error condition is set the first descriptor but the (CL!=3). The other error condition is there is a source descriptor but CL, KL, IL, PL, SHIVL are all 0. Write "1" to clear this error signal. */
#define IPSEC_BIT_SRC_ERR2                    ((u32)0x00000001 << 2)          /*!<R/W 0  Source Descriptor Error 2 Interrupt. One of the error condition is source descriptor starting address error (SDSA[1:0]!=2'd0 [At least 4 bytes-aligned]). The other error condition is memory protection event. Write "1" to clear this error signal. */
#define IPSEC_BIT_SRC_ERR1                    ((u32)0x00000001 << 1)          /*!<R/W 0  Source Descriptor Error 1 Interrupt. The error condition is last segment descriptor is set (LS=1), when descriptor is pointing to the first segment of the packet. Write "1" to clear this error signal. */
#define IPSEC_BIT_SRC_ERR0                    ((u32)0x00000001 << 0)          /*!<R/W 0  Source Descriptor Error 0 Interrupt. One of the error condition is first segment descriptor is not set (FS=0), when descriptor is pointing to the first segment of the packet. The other error condition is first segment descriptor is set (FS=1), when descriptor is not pointing to the first segment of the packet. Write "1" to clear this error signal. */
/** @} */
...
/** @defgroup IPSEKCR
 * @brief IPsec eFuse Key Control Register
 * @{
 **/
#define IPSEC_BIT_EFUSE_KEY_REQ               ((u32)0x00000001 << 31)          /*!<R/W 0  Request of loading key from OTP, cleared by key_rdy */
#define IPSEC_BIT_EFUSE_KEY_ID                ((u32)0x00000001 << 30)          /*!<R/W 0  1'b0: Load key from OTP key1 1'b1: Load key from OTP key2 */
/** @} */
...
#define IPSEC_MASK_FIFO_EMPTY_CNT             ((u32)0x000000FF << 0)          /*!<R 0  Destination Descriptor FIFO empty counter. Use this field to check how many available FIFO nodes in Destination descriptor FIFO could use. */
#define IPSEC_FIFO_EMPTY_CNT(x)               ((u32)(((x) & 0x000000FF) << 0))
#define IPSEC_GET_FIFO_EMPTY_CNT(x)           ((u32)(((x >> 0) & 0x000000FF)))
/** @} */
 ...
 * @{
 **/
#define IPSEC_MASK_DDFW                       ((u32)0xFFFFFFFF << 0)          /*!<R/W 0  Destination descriptor second word */
#define IPSEC_DDFW(x)                         ((u32)(((x) & 0xFFFFFFFF) << 0))
#define IPSEC_GET_DDFW(x)                     ((u32)(((x >> 0) & 0xFFFFFFFF)))
/** @} */

/** @defgroup DES_PKTCONF
 * @brief IPsec Descriptor Packet Configure Register
 * @{
 **/
 ...
 *****************************************************************************/
typedef struct {
	__IO uint32_t SDSR;                                   /*!< SOURCE DESCRIPTOR STATUS REGISTER,  Address offset: 0x0000 */
	__IO uint32_t SDFWR;                                  /*!< SOURCE DESCRIPTOR FIRST WORD REGISTER,  Address offset: 0x0004 */
	__IO uint32_t SDSWR;                                  /*!< SOURCE DESCRIPTOR SECOND WORD REGISTER,  Address offset: 0x0008 */
	__IO uint32_t RSVD0;                                  /*!<  Reserved,  Address offset:0xC */
	__IO uint32_t RSTEACONFISR;                           /*!< RESET ENGINE & CONFIGURE ISR REGISTER,  Address offset: 0x0010 */
	__IO uint32_t INTM;                                   /*!< INTERRUPT MASK REGISTER,  Address offset: 0x0014 */
	__IO uint32_t DBGR;                                   /*!< DEBUG REGISTER,  Address offset: 0x0018 */
	__IO uint32_t ERRSR;                                  /*!< ERROR STATUS REGISTER,  Address offset: 0x001C */
	__I  uint32_t SAADLR;                                 /*!< SUM OF ADDITIONAL AUTHENTICATION DATA LENGTH REGISTER,  Address offset: 0x0020 */
	__I  uint32_t SENLR;                                  /*!< SUM OF ENCRYPTION DATA LENGTH REGISTER,  Address offset: 0x0024 */
	__I  uint32_t SAPLR;                                  /*!< SUM OF AUTHENTICATION PADDING DATA LENGTH REGISTER,  Address offset: 0x0028 */
	__I  uint32_t SEPLR;                                  /*!< SUM OF ENCRYPTION PADDING DATA LENGTH REGISTER,  Address offset: 0x002C */
	__IO uint32_t SWAPABURSTR;                            /*!< SWAP AND BURST REGISTER,  Address offset: 0x0030 */
	__IO uint32_t IPSEKCR;                                /*!< IPSEC EFUSE KEY CONTROL REGISTER,  Address offset: 0x0034 */
	...
	__IO uint32_t DES_PKTCONF;                            /*!< IPSEC DESCRIPTOR PACKET CONFIGURE REGISTER,  Address offset: 0x100C */
...
/** @defgroup CRYPTO_Authentication_Type_Definitions
  * @{
  */
