 ...
 * @{
 **/
#define LCDC_BIT_FRM_START_INTRS          ((u32)0x00000001 << 5)          /*!<R 0x0  DMA frame start raw interrupt status. No matter DMA frame start interrupt is enabled or disabled, this bit is set when DMA frame start interrupt happens. Write 1 to the FRM_START_INTS field in register LCDC_IRQ_STATUS to clear this bit. */
#define LCDC_BIT_LCD_LIN_INTRS            ((u32)0x00000001 << 3)          /*!<R 0x0  Line Interrupt raw status. No matter the line interrupt is enabled or disabled, this bit is set when line interrupt happens. Write 1 to the LCD_LIN_INTS field in register LCDC_IRQ_STATUS to clear this bit. */
#define LCDC_BIT_LCD_FRD_INTRS            ((u32)0x00000001 << 2)          /*!<R 0x0  LCD refresh frame done raw interrupt status. No matter the refresh done interrupt is enabled or disabled, this bit is set when refresh done interrupt happens. Write 1 to the LCDFRDINTS field in register LCDC_IRQ_STATUS to clear this bit. */
 ...
 * @{
 **/
#define LCDC_MASK_CUR_POS_Y               ((u32)0x00000FFF << 16)          /*!<R 0x0  Current Y Position These bits return the current Y position. */
#define LCDC_CUR_POS_Y(x)                 ((u32)(((x) & 0x00000FFF) << 16))
#define LCDC_GET_CUR_POS_Y(x)             ((u32)(((x >> 16) & 0x00000FFF)))
#define LCDC_MASK_CUR_POS_X               ((u32)0x00000FFF << 0)          /*!<R 0x0  Current X Position These bits return the current X position. */
 ...
 * @{
 **/
#define LCDC_BIT_LCDCREADY                ((u32)0x00000001 << 31)          /*!<R 0x0  LCDC is ready to operate with VO interface after LCDC is enable. */
 ...
 * @{
 **/
#define LCDC_MASK_LAYERx_IMG_FORMAT       ((u32)0x00000007 << 5)          /*!<R/W 0x0  Image layer data format, it supports following ones: *000: ARGB8888 *001: RGB888 *010: RGB565 *011: ARGB1555 *100: ARGB4444 *101: RGB666 *110: ARGB8666 *Others: Reserved */
 ...
 * @{
 **/
#define LCDC_MASK_LAYERx_WIN_X_STOP       ((u32)0x00000FFF << 16)          /*!<R/W 0x0  Window Horizontal Stop Position These bits configure the last visible pixel of a line of the layer window. Please make sure it is in work plane */
#define LCDC_LAYERx_WIN_X_STOP(x)         ((u32)(((x) & 0x00000FFF) << 16))
#define LCDC_GET_LAYERx_WIN_X_STOP(x)     ((u32)(((x >> 16) & 0x00000FFF)))
#define LCDC_MASK_LAYERx_WIN_X_START      ((u32)0x00000FFF << 0)          /*!<R/W 0x0  Window Horizontal Start Position These bits configure the first visible pixel of a line of the layer window. Please make sure it is in work plane. */
 ...
 * @{
 **/
#define LCDC_MASK_LAYERx_WIN_Y_STOP       ((u32)0x00000FFF << 16)          /*!<R/W 0x0  Window Vertical Stop Position These bits configure the last visible line of the layer window. Please make sure it is in work plane. */
#define LCDC_LAYERx_WIN_Y_STOP(x)         ((u32)(((x) & 0x00000FFF) << 16))
#define LCDC_GET_LAYERx_WIN_Y_STOP(x)     ((u32)(((x >> 16) & 0x00000FFF)))
#define LCDC_MASK_LAYERx_WIN_Y_START      ((u32)0x00000FFF << 0)          /*!<R/W 0x0  Window Vertical Start Position These bits configure the first visible line of the layer window. Please make sure it is in work plane. */
 ...
 * @{
 **/
#define LCDC_MASK_LAYERx_BF1              ((u32)0x00000003 << 8)          /*!<R/W 0x0  blending factor 1 These bits select the blending factor F1. *00: constant alpha *01: pixel alpha x constant alpha *Others : Reserved */
#define LCDC_LAYERx_BF1(x)                ((u32)(((x) & 0x00000003) << 8))
#define LCDC_GET_LAYERx_BF1(x)            ((u32)(((x >> 8) & 0x00000003)))
#define LCDC_MASK_LAYERx_CONSTA           ((u32)0x000000FF << 0)          /*!<R/W 0xff  constant alpha These bits configure the Constant Alpha used for blending. The Constant Alpha is divided by 256 by hardware. The max value of Constant is FF,so when Constant Alpha arrive max value, it is identified as 1. */
...
typedef struct {
	...
	__IO uint32_t RSVD1[18];                              /*!<  Reserved,  Address offset:0x38-0x7C */
...
/**
 * @brief LCDC Layer Initialization structure definition
 */
typedef struct {
	u8 LCDC_LayerEn;				/*!< Specifies layer enable or not
                                      This parameter can be a value of ENABLE or DISABLE*/
	u8 LCDC_LayerImgFormat;			/*!< Specifies the layer image data format.
                                      This parameter can be a value of @ref LCDC_LAYER_IMG_FORMAT_define */
...
/** @defgroup LCDC_DMA_DEBUG_OUT_define
  * @{
  */
...
/** @defgroup LCDC_LAYER_IMG_FORMAT_define
  * @{
  */
...
/**
 * @brief LCDC Initialization structure definition
 */
typedef struct {
	u16 LCDC_ImageWidth;
