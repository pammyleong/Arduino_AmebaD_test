...
/* group: SHA_Patch_Functions */
/* Since all the members in the group are __STATIC_INLINE which will not be extracted to the doxygen doc,
  original comment has been delete to avoid there is nothing displayed under the group. */

__STATIC_INLINE
int rtl_crypto_sha2_update(u8 *message, u32 msglen,  hw_sha_context *ctx)
{
...
__STATIC_INLINE
int rtl_crypto_sha1_update(u8 *message, u32 msglen,  hw_sha_context *ctx)
{
	int ret;
	while (msglen > 128 * 1024) {
...
__STATIC_INLINE
int rtl_crypto_hmac_sha1_update(u8 *message, u32 msglen,  hw_sha_context *ctx)
{
	int ret;
	while (msglen > 128 * 1024) {
...
__STATIC_INLINE
int rtl_crypto_hmac_sha2_update(u8 *message, u32 msglen,  hw_sha_context *ctx)
{
	int ret;
...
__STATIC_INLINE
int rtl_crypto_md5_update(u8 *message, u32 msglen,  hw_sha_context *ctx)
{
	int ret;
