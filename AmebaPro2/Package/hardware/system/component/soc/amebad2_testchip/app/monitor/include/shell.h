...
#ifndef AMEBAD2_TODO
/* These defination is used for UART rx as loguart rx patch */
#ifdef CONFIG_UART_RX_PATCH
u32 shell_cmd_exec(u8  argc, u8  **argv);
VOID shell_task_ram_alternative(VOID *Data);
#endif

//#define CONFIG_UART_TX_PATCH
#ifdef CONFIG_UART_TX_PATCH
u32 DiagPrintf_p(IN  const char *fmt, ...);

#define DBG_AMEBAD2(...)     do {\
		if (unlikely(ConfigDebug[LEVEL_ERROR] & BIT(MODULE_MISC))) \
			DiagPrintf_p("[amebad2]#\r" __VA_ARGS__);\
	}while(0)
#endif

/* for NP--> LP and NP-->AP ipc rx handler */
void shell_uratRx_ipc_int(VOID *Data, u32 IrqStatus, u32 ChanNum);
#endif
