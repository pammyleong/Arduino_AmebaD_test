...
typedef struct _RAM_OTG_FUNCTION_TABLE_ {
	VOID (*RamMemSet)(void *dest, int byte, u32 size);
	VOID (*RamMemCpy)(void *dest, void *src, u32 size);
	int (*RamMemCmp)(void *m1, void *m2, u32 size);
	...
	int (*RamVsnPrintf)(char *str, size_t size, const char *format, ...);
	u32(*RamSPrintf)(u8 *buf, const char *fmt, ...);
	int (*RamSnPrintf)(char *dst, size_t count, const char *src, ...);
