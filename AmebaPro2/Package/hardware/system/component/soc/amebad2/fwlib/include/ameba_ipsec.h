 ...
 * @{
 *****************************************************************************/
#define IPSEC_BIT_DES_ERR5                    ((u32)0x00000001 << 15)          /*!<R/W1C 0  Destination Descriptor Error 5 Interrupt. The error condition is authentication digest length error in destination descriptor. MD5: adl != 16 or SHA1: adl != 20 or SHA2-224 adl != 28 or adl != 32 when set sequential hash and not sequential hash last SHA2-256 adl != 32 or SHA2-384 adl != 48 or adl != 64 when set sequential hash and not sequential hash last SHA2-512 adl != 64 Write '1' to clear this error signal. */
#define IPSEC_BIT_DES_ERR4                    ((u32)0x00000001 << 14)          /*!<R/W1C 0  Destination Descriptor Error 4 Interrupt. The error condition is authentication digest length error in destination descriptor. MD5: adl > 16 or SHA1: adl > 20 or SHA2-224: adl > 28 or adl > 32when set sequential hash and not sequential hash last SHA2-256 : adl > 32 or SHA2-384 : adl > 48 or adl > 64when set sequential hash and not sequential hash last SHA2-512 : adl > 64 Write '1' to clear this error signal. */
#define IPSEC_BIT_DES_ERR3                    ((u32)0x00000001 << 13)          /*!<R/W1C 0  Destination Descriptor Error 3 Interrupt. One of the error condition is the sum of enl is greater than Encryption Total Length. The other error condition is about destination descriptor length error ((enl(Encryption length)==0) & (adl(authentication data length)==0)). Write '1' to clear this error signal. Sum of enl reference the register REG_IPSCSR_SENLR (offset 0x24). */
#define IPSEC_BIT_DES_ERR2                    ((u32)0x00000001 << 12)          /*!<R/W1C 0  Destination Descriptor Error 2 Interrupt. The error condition is the bit field ENC=1'b0( active Hash engine to process hash data), but only active Cipher engine. Write '1' to clear this error signal. */
#define IPSEC_BIT_DES_ERR1                    ((u32)0x00000001 << 11)          /*!<R/W1C 0  Destination Descriptor Error 1 Interrupt. The error condition is the bit field ENC=1'b1( active cipher engine to process cipher data), but only active Hash engine. Write '1' to clear this error signal. */
#define IPSEC_BIT_DES_ERR0                    ((u32)0x00000001 << 10)          /*!<R/W1C 0  Destination Descriptor Error 0 Interrupt. One of the error condition is destination descriptor starting address error (DDSA[1:0]!=2'd0 [At least 4 bytes-aligned]). The other error condition is memory protection event. Write '1' to clear this error signal. */
#define IPSEC_BIT_SRC_ERR9                    ((u32)0x00000001 << 9)          /*!<R/W1C 0  Source Descriptor Error 9 Interrupt. The error condition is sum of epl is not equal to EPTL(Encryption Padding Total Length). Write '1' to clear this error signal. Sum of epl reference the register REG_IPSCSR_SEPLR (offset 0x2C). */
#define IPSEC_BIT_SRC_ERR8                    ((u32)0x00000001 << 8)          /*!<R/W1C 0  Source Descriptor Error 8 Interrupt. The error condition is sum of apl is not equal to HPTL(Hash Padding Total Length). Write '1' to clear this error signal. Sum of apl reference the register REG_IPSCSR_SAPLR (offset 0x28). */
#define IPSEC_BIT_SRC_ERR7                    ((u32)0x00000001 << 7)          /*!<R/W1C 0  Source Descriptor Error 7 Interrupt. The error condition is sum of enl is not equal to ETL(Encryption Total Length). Write '1' to clear this error signal. Sum of enl reference the register REG_IPSCSR_SENLR (offset 0x24). */
#define IPSEC_BIT_SRC_ERR6                    ((u32)0x00000001 << 6)          /*!<R/W1C 0  Source Descriptor Error 6 Interrupt. The error condition is sum of a2eo is not equal to HTL(Header Total Length). Write '1' to clear this error signal. Sum of a2eo reference the register REG_IPSCSR_SAADLR (offset 0x20). */
#define IPSEC_BIT_SRC_ERR5                    ((u32)0x00000001 << 5)          /*!<R/W1C 0  Source Descriptor Error 5 Interrupt. The error condition is there is a source descriptor, but the data length( (a2eo/epl) +(enl) + (apl)) is 0. Write '1' to clear this error signal. */
#define IPSEC_BIT_SRC_ERR4                    ((u32)0x00000001 << 4)          /*!<R/W1C 0  Source Descriptor Error 4 Interrupt. The error condition is source data buffer pointer error(SDBP[1:0]!=2'd0[At least 4 byte aligned]). Write '1' to clear this error signal. */
#define IPSEC_BIT_SRC_ERR3                    ((u32)0x00000001 << 3)          /*!<R/W1C 0  Source Descriptor Error 3 Interrupt. About source descriptor length error. One of the error condition is set the first descriptor but the (CL!=3). The other error condition is there is a source descriptor but CL, KL, IL, PL, SHIVL are all 0. Write '1' to clear this error signal. */
#define IPSEC_BIT_SRC_ERR2                    ((u32)0x00000001 << 2)          /*!<R/W1C 0  Source Descriptor Error 2 Interrupt. One of the error condition is Source descriptor starting address error (SDSA[1:0]!=2'd0 [At least 4 bytes-aligned]). The other error condition is memory protection event. Write '1' to clear this error signal. */
#define IPSEC_BIT_SRC_ERR1                    ((u32)0x00000001 << 1)          /*!<R/W1C 0  Source Descriptor Error 1 Interrupt. The error condition is last Segment descriptor is set (LS=1), when descriptor is pointing to the first segment of the packet. Write '1' to clear this error signal. */
#define IPSEC_BIT_SRC_ERR0                    ((u32)0x00000001 << 0)          /*!<R/W1C 0  Source Descriptor Error 0 Interrupt. One of the error condition is first segment descriptor is not set (FS=0), when descriptor is pointing to the first segment of the packet. The other error condition is first segment descriptor is set (FS=1), when descriptor is not pointing to the first segment of the packet. Write '1' to clear this error signal. */
/** @} */
...
/**************************************************************************//**
 * @defgroup AMEBAD2_IPSEC
 * @{
 * @brief AMEBAD2_IPSEC Register Declaration
 *****************************************************************************/
typedef struct {
