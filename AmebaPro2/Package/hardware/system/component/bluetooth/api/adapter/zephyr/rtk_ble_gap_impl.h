...
rtk_bt_evt_status_t rtk_ble_gap_get_address_impl(rtk_ble_bd_addr_t* addr);
...
rtk_bt_evt_status_t rtk_ble_gap_get_whitelist_size_impl(uint16_t *len);

rtk_bt_evt_status_t rtk_ble_gap_read_rssi_impl(uint8_t conn_id, int32_t* rssi);

rtk_bt_evt_status_t rtk_ble_gap_get_conn_info_impl(uint8_t conn_id, rtk_ble_conn_info_t* info);

rtk_bt_evt_status_t rtk_ble_gap_set_channels_impl(rtk_ble_channels_t chan_map);

rtk_bt_evt_status_t rtk_ble_gap_set_default_data_len_impl(rtk_ble_data_len_t* param);
...
rtk_bt_err_t rtk_ble_sm_pairing_confirm_impl(rtk_ble_pair_cfm_t *param);
...
rtk_bt_evt_status_t rtk_ble_sm_get_bond_num_impl(uint8_t *bond_num);

rtk_bt_evt_status_t rtk_ble_sm_get_bond_info_impl(rtk_ble_bond_info_t *info);
