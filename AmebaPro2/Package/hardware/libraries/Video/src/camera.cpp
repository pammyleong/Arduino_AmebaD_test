...
void Video::videoInit(void) {
    int heapSize = cameraConfigNew(channelEnable[0], w[0], h[0], bps[0], snapshot[0],
...
void Video::channelBegin(int ch) {
    switch (ch) {
        ...
        case 2: {
            cameraStart(videoModule[ch]._p_mmf_context->priv, channel[ch]);
            cameraSnapshot(videoModule[ch]._p_mmf_context->priv, channel[ch]);
...
/**
  ...
  * @retval none
  */
void Video::getP(VideoSetting& obj, bool cb_flag) {
    if (cb_flag == 0) {
