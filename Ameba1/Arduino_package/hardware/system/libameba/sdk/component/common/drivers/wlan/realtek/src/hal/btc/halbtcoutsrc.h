...
#if 1	// msg mutex lock
	#define		BTC_SPRINTF(buf, size, ...)		\
				do{								\
					if(ODM_COMP_BT_COEX & ROMInfo.DebugComponents){	\
						rtw_mutex_get(&btc_msg_mutex);	\
						rtl_printf(__VA_ARGS__);	\
						rtw_mutex_put(&btc_msg_mutex);	\
					}	\
				}while(0)
	#define		BTC_TRACE(_MSG_)
#else
	#define		BTC_SPRINTF				rtl_snprintf
	#define 	BTC_TRACE(_MSG_)									\
				do{													\
					if(ODM_COMP_BT_COEX & ROMInfo.DebugComponents){	\
						rtl_printf(_MSG_);							\
...
#define	CL_SPRINTF	  rtl_snprintf // rsprintf
#define	CL_PRINTF	  rtl_printf // DCMD_Printf
