...
void setup() {
    ...
    * When hardfault happened it makes timer handler cannot feed watchdog and watchdog would restart the system. */
    ...
    /* prepare LASS data */
    sprintf(payload,
            "|ver_format=3|FAKE_GPS=1|app=PM25|ver_app=%s|device_id=%s|date=%4d-%02d-%02d|time=%02d:%02d:%02d|s_t0=%3.1f|s_h0=%2.1f|gps_lon=%s|gps_lat=%s",
            "live",
            clientId,
            year(memdata->timestamp),
            month(memdata->timestamp),
            day(memdata->timestamp),
            hour(memdata->timestamp),
            minute(memdata->timestamp),
            second(memdata->timestamp),
            memdata->h,
            memdata->t,
            gps_location[1],
            gps_location[0]);
